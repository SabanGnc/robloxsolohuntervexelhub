...
local WebhookURL = "https://discord.com/api/webhooks/1466541943243800801/sIGhFbUwli1QI8ya46M0xCQ6KHgbIMN3RZxO3i_B-hQV_fF3MwiGkyemP4oBlt7OYkn1"

local function SendWebhook(title, description)
    task.spawn(function()
        local scriptTime = os.date("!%Y-%m-%dT%H:%M:%SZ")
        local embed = {
            ["title"] = title,
            ["description"] = description .. "\n\n**Kullanıcı:** " .. LocalPlayer.Name,
            ["color"] = 7536895, 
            ["footer"] = {
                ["text"] = "Vexel HUB v11.1 • Log Sistemi"
            },
        ...
        local data = {
            ["username"] = "Vexel HUB",
            ["avatar_url"] = "https://i.imgur.com/8Q9Q1Q0.png",
            ["embeds"] = {embed}
        }
...
local Teleport_Places = {
    ["Buraya Sat"] = Vector3.new(-22, -10, -76),
    ["Tüccar"] = Vector3.new(-43, -10, -16),
    ["Günlük Görevler"] = Vector3.new(89, -10, -82),
    ["Büyüler"] = Vector3.new(804, -9, 505)
}

local Teleport_NPCs = {
    ["Hunter Tin"] = Vector3.new(-84, -10, -14),
    ["Rubble Doug"] = Vector3.new(22, -10, -398),
    ["Hunter Blaze"] = Vector3.new(-986, -10, -428),
    ["Hunter Ki"] = Vector3.new(368, -10, 311),
    ["Codes"] = Vector3.new(-64, -10, 1443),
    ["Hunter Frost John"] = Vector3.new(124, -10, 1437)
}
...
if CoreGui:FindFirstChild("VexelHubUI") then
    CoreGui:FindFirstChild("VexelHubUI"):Destroy()
end
if CoreGui:FindFirstChild("VexelIntro") then
    CoreGui.VexelIntro:Destroy()
end
if CoreGui:FindFirstChild("VexelTracers") then
    CoreGui.VexelTracers:Destroy()
end
if CoreGui:FindFirstChild("VexelDevESP") then
    CoreGui.VexelDevESP:Destroy()
end
...
local Config = {
    ...
    ClickDelete = false,
    ChatSpam = false, SpamMsg = "Vexel Hub v9.3!",
    ChatSpy = false, 
    AntiAFK = false, 
    AutoRejoin = false, 
    Spectating = false,
    ...
    SoloFarm = false,
    SoloDist = 5,
    SoloSearchRadius = 300,
    SoloPos = "Arkadan",
    AutoChest = false, 
    SelectedPlace = "Buraya Sat",
    SelectedNPC = "Hunter Tin",
...
local function FullShutdown()
    ...
    if LocalPlayer.Character then
        ...
        if root then
            if root:FindFirstChild("VexelLV") then root.VexelLV:Destroy() end
            if root:FindFirstChild("VexelAtt") then root.VexelAtt:Destroy() end
            if root:FindFirstChild("VexelAO") then root.VexelAO:Destroy() end
            if root:FindFirstChild("SpiderBV") then root.SpiderBV:Destroy() end
            root.Anchored = false
        end
        if hum then
            hum.WalkSpeed = 16
    ...
    SendWebhook("Sistem Kapatıldı", "Panic Button kullanıldı. Script sonlandırıldı.")
end
...
TitleLbl.Text = "Vexel Hub - <font color=\"rgb(150,50,255)\">v11.1</font>"
...
local Tab_Solo, Page_Solo = CreateTab("Solo Hunters") 
...
Tab_Solo.MouseButton1Click:Connect(function() SwitchTab(Tab_Solo, Page_Solo) end)
...
local function CreateToggle(parent, text, callback)
    ...
    local function SetState(val)
        ...
        callback(enabled)
        SendWebhook("Toggle İşlemi", "**" .. text .. "** durumu: " .. tostring(enabled))
    end
...
local function CreateDropdown(parent, text, options, default, callback)
    ...
    local Label = Instance.new("TextLabel")
    Label.Parent = Frame; Label.Text = text; Label.TextColor3 = Color3.fromRGB(255,255,255); Label.Font = Enum.Font.GothamBold; Label.TextSize = 14
    Label.Position = UDim2.new(0, 10, 0, 0); Label.Size = UDim2.new(0.5, 0, 0, 40); Label.BackgroundTransparency = 1; Label.TextXAlignment = Enum.TextXAlignment.Left
    
    local SelectedLabel = Instance.new("TextLabel")
    SelectedLabel.Parent = Frame; SelectedLabel.Text = default; SelectedLabel.TextColor3 = Color3.fromRGB(115,0,255); SelectedLabel.Font = Enum.Font.GothamBold; SelectedLabel.TextSize = 14
    SelectedLabel.Position = UDim2.new(0.5, 0, 0, 0); SelectedLabel.Size = UDim2.new(0.4, 0, 0, 40); SelectedLabel.BackgroundTransparency = 1; SelectedLabel.TextXAlignment = Enum.TextXAlignment.Right
    
    local Arrow = Instance.new("ImageLabel", Frame)
    Arrow.Size = UDim2.new(0, 20, 0, 20); Arrow.Position = UDim2.new(1, -30, 0, 10); Arrow.BackgroundTransparency = 1; Arrow.Image = "rbxassetid://6034818372"
    
    local Trigger = Instance.new("TextButton", Frame)
    Trigger.Size = UDim2.new(1, 0, 0, 40); Trigger.BackgroundTransparency = 1; Trigger.Text = ""
    
    local OptionContainer = Instance.new("ScrollingFrame", Frame)
    OptionContainer.Position = UDim2.new(0, 0, 0, 45); OptionContainer.Size = UDim2.new(1, 0, 0, 100); OptionContainer.BackgroundTransparency = 1; OptionContainer.ScrollBarThickness = 2
    local OptList = Instance.new("UIListLayout", OptionContainer); OptList.Padding = UDim.new(0, 2)
    ...
    local function Toggle()
        open = not open
        if open then
            TweenService:Create(Frame, TweenInfo.new(0.3), {Size = UDim2.new(0.95, 0, 0, 150)}):Play()
            TweenService:Create(Arrow, TweenInfo.new(0.3), {Rotation = 180}):Play()
        else
            TweenService:Create(Frame, TweenInfo.new(0.3), {Size = UDim2.new(0.95, 0, 0, 40)}):Play()
    ...
    for _, opt in pairs(options) do
        ...
        btn.Size = UDim2.new(1, -10, 0, 30); btn.BackgroundColor3 = Color3.fromRGB(40, 40, 45); btn.Text = opt; btn.TextColor3 = Color3.fromRGB(200, 200, 200); btn.Font = Enum.Font.Gotham; btn.TextSize = 13
        local c = Instance.new("UICorner", btn); c.CornerRadius = UDim.new(0, 6)
        btn.MouseButton1Click:Connect(function() Select(opt) end)
    end
    
    OptionContainer.AutomaticCanvasSize = Enum.AutomaticSize.Y
    
    table.insert(UI_Storage, {Type = "Dropdown", Name = text, Get = function() return currentVal end, Set = function(v) Select(v) end})
end
...
local function CreateButton(parent, text, callback)
    ...
    Button.MouseButton1Click:Connect(function()
        ...
        callback()
        SendWebhook("Buton Tıklandı", "**" .. text .. "** butonu kullanıldı.")
    end)
end
...
CreateToggle(Page_Main, "Speed (Standart)", function(v) 
    Config.SpeedOn = v
...
CreateToggle(Page_Physics, "Uçma (Fly) - STABIL", function(v) 
    Config.Fly = v 
...
CreateSlider(Page_Physics, "Fly Hızı", 10, 300, 50, function(v) Config.FlySpeed = v end)
...
CreateToggle(Page_Player, "İzle (Spectate)", function(v) Config.Spectating = v; if not v and LocalPlayer.Character then Camera.CameraSubject=LocalPlayer.Character.Humanoid end end)
...
CreateSection(Page_Solo, "Otomatik Farm")

CreateToggle(Page_Solo, "Auto Farm (Aktif/Pasif)", function(v) 
    Config.SoloFarm = v 
    if v then Notify("Auto Farm", "Başlatıldı! Mobs klasörü taranıyor...") end
end)

CreateToggle(Page_Solo, "Auto Chest (Dungeon Sonu)", function(v) Config.AutoChest = v end)

CreateDropdown(Page_Solo, "Atak Pozisyonu", {"Üstten", "Alttan", "Arkadan", "Önden"}, "Arkadan", function(v) Config.SoloPos = v end)
CreateSlider(Page_Solo, "Mesafe", 1, 20, 5, function(v) Config.SoloDist = v end)
CreateSlider(Page_Solo, "Maks. Tarama (Mesafe)", 100, 2000, 300, function(v) Config.SoloSearchRadius = v end) 

CreateSection(Page_Solo, "Mekan Işınlanma")
local PlacesList = {}
for name, _ in pairs(Teleport_Places) do table.insert(PlacesList, name) end
table.sort(PlacesList)
CreateDropdown(Page_Solo, "Mekan Seç", PlacesList, "Buraya Sat", function(v) Config.SelectedPlace = v end)
CreateButton(Page_Solo, "Mekana Işınlan", function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and Config.SelectedPlace then
        local targetPos = Teleport_Places[Config.SelectedPlace]
        if targetPos then
            LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(targetPos + Vector3.new(0, 3, 0))
            Notify("Işınlanma", Config.SelectedPlace .. " konumuna gidildi.")
            SendWebhook("Teleport İşlemi", "Mekan: " .. Config.SelectedPlace)
        end
    else
        Notify("Hata", "Karakter yok veya hedef seçilmedi.")
    end
end)

CreateSection(Page_Solo, "NPC Işınlanma")
local NpcList = {}
for name, _ in pairs(Teleport_NPCs) do table.insert(NpcList, name) end
table.sort(NpcList)
CreateDropdown(Page_Solo, "NPC Seç", NpcList, "Hunter Tin", function(v) Config.SelectedNPC = v end)
CreateButton(Page_Solo, "NPC'ye Işınlan", function()
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and Config.SelectedNPC then
        local targetPos = Teleport_NPCs[Config.SelectedNPC]
        if targetPos then
            LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(targetPos + Vector3.new(0, 3, 0))
            Notify("Işınlanma", Config.SelectedNPC .. " konumuna gidildi.")
            SendWebhook("Teleport İşlemi", "NPC: " .. Config.SelectedNPC)
        end
    else
        Notify("Hata", "Karakter yok veya hedef seçilmedi.")
    end
...
TargetStatus.Parent = Page_Solo
TargetStatus.Size = UDim2.new(0.95, 0, 0, 30)
TargetStatus.BackgroundTransparency = 1
TargetStatus.Text = "Durum: Bekleniyor..."
...
table.insert(Connections, task.spawn(function()
    while true do
        task.wait() 
        if Config.SoloFarm and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            if not LocalPlayer.Character:FindFirstChild("Head") or not LocalPlayer.Character:FindFirstChild("Torso") and not LocalPlayer.Character:FindFirstChild("UpperTorso") then
                task.wait(1)
                continue
            end
            ...
            if not CurrentTarget or not CurrentTarget.Parent or not CurrentTarget:FindFirstChild("Humanoid") or CurrentTarget.Humanoid.Health <= 0 then
                ...
                if MobsFolder then
                    ...
                    for _, mob in pairs(MobsFolder:GetChildren()) do
                        if mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 and mob:FindFirstChild("HumanoidRootPart") then
                            ...
                            if dist < closestDist and dist < Config.SoloSearchRadius then
                                closestDist = dist
                                CurrentTarget = mob
                                FoundMob = true
                            end
                ...
                if not FoundMob then
                    if Config.AutoChest then
                        TargetStatus.Text = "Durum: Sandık Aranıyor..."
                        ...
                        for _, v in pairs(workspace:GetDescendants()) do
                            if v.Name:find("Chest") then
                                ...
                                if prompt and prompt.Enabled then
                                    local root = v:FindFirstChild("RootPart") or v.PrimaryPart or v:FindFirstChildWhichIsA("BasePart")
                                    if root and (root.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 2000 then 
                                        Chest = v
                                        break 
                                    end
                                end
                        ...
                        if Chest then
                            TargetStatus.Text = "Durum: Sandık Toplanıyor!"
                            
                            local chestPart = Chest:FindFirstChildWhichIsA("ProximityPrompt", true).Parent
                            if chestPart then
                                LocalPlayer.Character.HumanoidRootPart.CFrame = chestPart.CFrame * CFrame.new(0, 4, 0)
                                LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,0,0)
                                
                                for _, prompt in pairs(Chest:GetDescendants()) do
                            ...
                            TargetStatus.Text = "Durum: Tüm Sandıklar Açıldı."
                            TargetStatus.TextColor3 = Color3.fromRGB(50, 255, 50)
                        end
                    else
                        TargetStatus.Text = "Durum: Hedef Yok (Bekliyor)"
            ...
            if CurrentTarget and CurrentTarget:FindFirstChild("HumanoidRootPart") then
                ...
                TargetStatus.Text = "Hedef: " .. CurrentTarget.Name .. " [HP: " .. math.floor(hum.Health) .. "]"
                TargetStatus.TextColor3 = Color3.fromRGB(255, 50, 50)

                local mobSize = CurrentTarget:GetExtentsSize()
                local safeDistance = Config.SoloDist + (math.max(mobSize.X, mobSize.Z) / 2) 

                local offset = CFrame.new(0, 0, safeDistance)
                
                if Config.SoloPos == "Önden" then
                    offset = CFrame.new(0, 0, -safeDistance)
                elseif Config.SoloPos == "Üstten" then
                    offset = CFrame.new(0, safeDistance, 0) * CFrame.Angles(math.rad(-90), 0, 0)
                elseif Config.SoloPos == "Alttan" then
                    offset = CFrame.new(0, -safeDistance, 0) * CFrame.Angles(math.rad(90), 0, 0)
                end

                local targetPos = mobRoot.CFrame * offset
                
                LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(targetPos.Position, mobRoot.Position)
                LocalPlayer.Character.HumanoidRootPart.Velocity = Vector3.new(0,0,0)
            ...
            CurrentTarget = nil
            if not Config.SoloFarm then 
                TargetStatus.Text = "Durum: Devre Dışı" 
                TargetStatus.TextColor3 = Color3.fromRGB(255, 200, 50)
            end
        end
    end
...
table.insert(Connections, RunService.RenderStepped:Connect(function(deltaTime)
    ...
    if Config.ESP_Highlight or Config.ESP_NameTag or Config.ESP_Health then
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if dist < Config.ESP_MaxDist then
                    if Config.ESP_Highlight and not p.Character:FindFirstChild("VexelHighlight") then
                        local hl = Instance.new("Highlight", p.Character); hl.Name = "VexelHighlight"; hl.FillColor = Color3.fromRGB(115, 0, 255); hl.OutlineColor = Color3.fromRGB(255, 255, 255); hl.FillTransparency = 0.5; hl.OutlineTransparency = 0
                    end
                    if (Config.ESP_NameTag or Config.ESP_Health) and p.Character:FindFirstChild("Head") then
                        if Config.ESP_NameTag and not p.Character.Head:FindFirstChild("VexelNameTag") then
                            local bg = Instance.new("BillboardGui", p.Character.Head); bg.Name = "VexelNameTag"; bg.Size = UDim2.new(0, 200, 0, 50); bg.StudsOffset = Vector3.new(0, 2, 0); bg.AlwaysOnTop = true
                            local txt = Instance.new("TextLabel", bg); txt.Size = UDim2.new(1, 0, 1, 0); txt.BackgroundTransparency = 1; txt.Text = p.Name; txt.TextColor3 = Color3.fromRGB(255, 255, 255); txt.TextStrokeColor3 = Color3.fromRGB(115, 0, 255); txt.TextStrokeTransparency = 0; txt.Font = Enum.Font.GothamBold; txt.TextSize = 14
                        end
                        if Config.ESP_Health and p.Character.Head:FindFirstChild("VexelNameTag") then
                            local bg = p.Character.Head.VexelNameTag
                            local hb = bg:FindFirstChild("HealthBar") or Instance.new("Frame", bg); hb.Name = "HealthBar"; hb.BackgroundColor3 = Color3.fromRGB(50, 50, 50); hb.BorderSizePixel = 0; hb.Size = UDim2.new(0.6, 0, 0, 4); hb.Position = UDim2.new(0.2, 0, 0.85, 0)
                            local fill = hb:FindFirstChild("Fill") or Instance.new("Frame", hb); fill.Name = "Fill"; fill.BorderSizePixel = 0
                            local hum = p.Character:FindFirstChild("Humanoid")
                            if hum then local health = math.clamp(hum.Health / hum.MaxHealth, 0, 1); fill.Size = UDim2.new(health, 0, 1, 0); fill.BackgroundColor3 = Color3.fromRGB(255, 0, 0):Lerp(Color3.fromRGB(0, 255, 0), health) end
                        end
                    end
                else
                    if p.Character:FindFirstChild("VexelHighlight") then p.Character.VexelHighlight:Destroy() end
    ...
    if LocalPlayer.Character then
        ...
        if hum and root then
            ...
            if Config.CFrameWalk and hum.MoveDirection.Magnitude > 0 then
            ...
            if Config.Fly then
                local att = root:FindFirstChild("VexelAtt") or Instance.new("Attachment", root); att.Name="VexelAtt"
                local lv = root:FindFirstChild("VexelLV") or Instance.new("LinearVelocity", root); lv.Name="VexelLV"
                local ao = root:FindFirstChild("VexelAO") or Instance.new("AlignOrientation", root); ao.Name="VexelAO"
                ...
                lv.VectorVelocity = vel * Config.FlySpeed
            ...
            if Config.VehicleFly and hum.SeatPart then local veh = hum.SeatPart.Parent; if veh and veh.PrimaryPart then local bv = veh.PrimaryPart:FindFirstChild("VehFly") or Instance.new("BodyVelocity", veh.PrimaryPart); bv.Name="VehFly"; bv.MaxForce=Vector3.new(1e9,1e9,1e9); local camCF = Camera.CFrame; local vel = Vector3.new(); if UserInputService:IsKeyDown(Enum.KeyCode.W) then vel = vel + camCF.LookVector end; if UserInputService:IsKeyDown(Enum.KeyCode.S) then vel = vel - camCF.LookVector end; if UserInputService:IsKeyDown(Enum.KeyCode.Space) then vel = vel + Vector3.new(0,1,0) end; bv.Velocity = vel * Config.VehFlySpeed end end
        end
    end
